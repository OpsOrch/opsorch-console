...
export async function GET(request: Request) {
  if (!isEnterprise()) {
    return NextResponse.json(
      { error: "Chat search is only available in Enterprise Edition" },
      { status: 404 }
    );
  }
  ...
  try {
    ...
    const res = await fetch(
      `${COPILOT_BASE_URL}/chats/search?${queryString}`,
      {
        method: "GET",
    ...
    const message = err instanceof Error ? err.message : "Failed to search chats";
